...
const Booking = () => {
  ...
  const { data, loading, error } = useFetchData(
    user ? `${process.env.NEXT_PUBLIC_BASE_URL}/reservation/name/${user.name}` : null
  );

  // Ensure fetched data is always an array
  useEffect(() => {
    if (data) {
      setFetchedReservations(Array.isArray(data) ? data : []);
    }
  }, [data]);
  ...
  const isDeleteDisabled = (checkOutDate) => {
    const currentDate = new Date(); // Replace with real current date in production
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedData = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/reservation/user/${user._id}`
        ).then((res) => res.json());
        setFetchedReservations(Array.isArray(updatedData) ? updatedData : []); // Ensure array
            ...
            return (
              <tr key={reservation._id}>
                ...
                <td className="px-6 py-4">
                  <button
                    onClick={() => navigateToPropertyDetail(accommodationId)}
        ...
        {fetchedReservations.map((reservation) => {
          ...
          return (
            <div key={reservation._id} className="pb-4 mb-4 border-b">
              ...
              <p>
                <strong>Persons:</strong> {reservation.numberOfPersons}
              </p>
              ...
              <p>
                <strong>Status:</strong> {reservation.isApproved}
              </p>
              <button
                onClick={() => navigateToPropertyDetail(accommodationId)}
                className="inline-block px-6 py-3 mt-4 text-white bg-blue-500 border rounded-xl"
              >
