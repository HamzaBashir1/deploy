...
const FeatureCard = () => {
  ...
  const incrementViewCount = async (id) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/accommodation/${id}/view`,
        {
          method: "PUT",
          headers: {
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {accommodationData &&
            accommodationData.map((property) => {
              const ratingsInfo =
                ratingsData[property._id] || { averageRating: 0, ratingsCount: 0 };
                ...
                >
                  ...
                  <div className="p-3 sm:p-4">
                    ...
                    <p className="text-lg sm:text-sm text-[#666666]">
                      {property.person} persons, {property.bedroomCount} bedrooms,{" "}
                      ...
                      Array.isArray(property.equipmentAndServices) && (
                        <div className="flex flex-wrap gap-2 mt-2 sm:mt-3">
                          {property.equipmentAndServices.includes("Parking") && (
                            <MdLocalParking className="text-base sm:text-lg" />
                          )}
                          {property.equipmentAndServices.includes("Wifi") && (
                            <IoWifi className="text-base sm:text-lg" />
                          )}
                          {property.equipmentAndServices.includes("Pet Friendly") && (
                            <MdOutlinePets className="text-base sm:text-lg" />
                          )}
                          {property.equipmentAndServices.includes("Pool") && (
                            <LuWaves className="text-base sm:text-lg" />
                          )}
                        </div>
                      )}
