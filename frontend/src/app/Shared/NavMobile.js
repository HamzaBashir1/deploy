...
const NavMobile = ({ data = DUMMY_DATA, onClickClose }) => {
      ...
      <ul className="nav-mobile-sub-menu pl-6 pb-1 text-base">
        {children.map((child) => (
          <li key={child.id}>
            <Link
              href={child.href || "#"}
              className="block px-4 py-2.5 text-neutral-900 text-sm font-medium rounded-lg hover:bg-neutral-100"
            >
              {child.name}
            </Link>
          </li>
  ...
  const renderItem = (item) => (
    <Disclosure key={item.id} as="li" className="text-neutral-900">
      {({ open }) => (
        <>
          <div className="flex items-center w-full px-4 font-medium uppercase tracking-wide text-sm hover:bg-neutral-100 rounded-lg">
            <Link
              href={item.href || "#"}
