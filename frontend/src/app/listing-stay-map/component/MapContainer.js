...
//   } from "@vis.gl/react-google-maps";
  ...
  const MapContainer = ({ currentHoverID = -1, DEMO_DATA, listingType }) => {
    return (
      <>
          ...
          {DEMO_DATA.map((item) => (
            ...
            >
              <AnyReactComponent
                ...
                lat={item.map.lat}
                lng={item.map.lng}
                car={listingType === "car" ? item : undefined}
                experiences={listingType === "experiences" ? item : undefined}
                listing={listingType === "stay" ? item : undefined}
              />
            </AdvancedMarker>
          ))}
        </Map> */}
